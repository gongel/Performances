...
enc_layer_mapping_dict = OrderedDict({
    ...
    "self_attn q_proj weight&&self_attn k_proj weight&&self_attn v_proj weight&&expression_.transpose([0, 1])",
    ...
    "ffn_first_kernel": "linear1 weight&&expression_.transpose([0, 1])",
    "ffn_first_bias": "linear1 bias",
    "ffn_second_kernel": "linear2 weight&&expression_.transpose([0, 1])",
    "ffn_second_bias": "linear2 bias",
})

dec_layer_mapping_dict = OrderedDict({
    "self_norm_scale": "norm1 weight",
    "self_norm_bias": "norm1 bias",
    ...
    "encdec_norm_scale": "norm2 weight",
    "encdec_norm_bias": "norm2 bias",
    "encdec_project_kernel_q":
    "cross_attn q_proj weight&&expression_.transpose([0, 1])",
    "encdec_project_bias_q": "cross_attn q_proj bias",
    "encdec_project_kernel_output":
    "cross_attn out_proj weight&&expression_.transpose([0, 1])",
    "encdec_project_bias_output": "cross_attn out_proj bias",
    "ffn_norm_scale": "norm3 weight",
    "ffn_norm_bias": "norm3 bias",
    "ffn_first_kernel": "linear1 weight&&expression_.transpose([0, 1])",
    "ffn_first_bias": "linear1 bias",
    "ffn_second_kernel": "linear2 weight&&expression_.transpose([0, 1])",
    "ffn_second_bias": "linear2 bias",
...
trg_emb_mapping_dict = OrderedDict({
    "norm_scale": "norm weight",
    "norm_bias": "norm bias",
    "shared_bias": "embedding_bias",
})


def _get_encode_output_mapping_dict(dec_layer_num):
    encode_output_kernel_pattern = [
        "cross_attn {0} k_proj weight&&cross_attn {0} v_proj weight".format(ele)
        for ele in range(dec_layer_num)
    ]
    encode_output_bias_pattern = [
        "cross_attn {0} k_proj bias&&cross_attn {0} v_proj bias".format(ele)
        for ele in range(dec_layer_num)
    ]
...
def save_bart_proto_to_hdf5(transformer: Transformer, f: h5py.File):
    ...
    f.create_dataset(
        "model_conf/n_encoder_stack", data=len(transformer.encoder_stack))
    f.create_dataset(
        "model_conf/n_decoder_stack", data=len(transformer.decoder_stack))
...
def extract_transformer_weights(
    ...
    # load var names
    reloaded = paddle.load(model_dir)
    with open('paddle_transformers.txt', 'w', encoding='utf8') as f:
        for k, v in reloaded.items():
            f.write(f'{k}\t{v.shape}\n')
    vocab_size = reloaded['trg_word_embedding.word_embedding.weight'].shape[0]
    reloaded['transformer.decoder.embedding_bias'] = paddle.create_parameter(
        shape=[vocab_size], dtype="float32", is_bias=True)
    ...
    for k in reloaded:
        ...
        if k == "src_word_embedding.word_embedding.weight" or k == "src_pos_embedding.pos_encoder.weight":
            encoder_state_dict[k] = reloaded[k]
        if k == "trg_word_embedding.word_embedding.weight" or k == "trg_pos_embedding.pos_encoder.weight":
            decoder_state_dict[k] = reloaded[k]
        if k == "decoding_linear.bias":
            decoder_state_dict[k] = reloaded[k]
    ...
    if not only_decoder:
        ...
        pos_emb_list = (
            encoder_state_dict["src_pos_embedding.pos_encoder.weight"]
            .numpy()[:max_step, :].reshape([-1]).tolist())
        transformer.src_embedding.position_embedding[:] = pos_emb_list
        print(
            "src_pos_embedding.pos_encoder.weight -> src_embedding.position_embedding, shape: {}, conversion finished!".
            format(encoder_state_dict["src_pos_embedding.pos_encoder.weight"]
                   .numpy()[:max_step, :].shape))
    ...
    pos_emb_list = (decoder_state_dict["trg_pos_embedding.pos_encoder.weight"]
                    .numpy()[:max_step, :].reshape([-1]).tolist())
    transformer.trg_embedding.position_embedding[:] = pos_emb_list
    print(
        "trg_pos_embedding.pos_encoder.weight -> trg_embedding.position_embedding, shape: {}, conversion finished!".
        format(decoder_state_dict["trg_pos_embedding.pos_encoder.weight"]
               .numpy()[:max_step, :].shape))
    ...
    trg_tb, _ = gather_token_embedding(
        ...
        "trg_word_embedding",
        scale=False)
    transformer.trg_embedding.token_embedding[:] = trg_tb.transpose().flatten(
    ).tolist()
    print(
        "token_embedding.weight -> trg_embedding.token_embedding, shape: {}, conversion finished!".
        format(trg_tb.transpose().shape))
...
if __name__ == "__main__":
    ...
    output_lightseq_model_name = f"lightseq_transformer_bs_{beam_size}_len_{max_step}"  # you can rename it to "lightseq_bart_large" for large model
    input_huggingface_bart_model = os.path.join(
        '/home/for_pr/PaddleNLP_test_pr/paddlenlp/ops/faster_transformer/',
        "transformer_no_encoder.pdparams")
    head_number = 8
    # in order to get score, we should use `beam_search` inference method
    generation_method = "beam_search"
    extra_decode_length = max_step  # max step for generation, it decides GPU memory occupancy
